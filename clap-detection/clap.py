class ClapAnalyzer:
    def __init__(self, note_lengths, deviation_threshold=0.1, double_clap_min_gap=0.08, double_clap_max_gap=0.4):
        """
        ...
        :param double_clap_min_gap: Min seconds between two claps to count as double clap
        :param double_clap_max_gap: Max seconds between two claps to count as double clap
        """
        ...
        self.min_pattern_time = .1 * self.pattern_sum
        self.max_pattern_time = .5 * self.pattern_sum
        self.clap_times = [None] * self.buffer_size
        self.deviation_threshold = deviation_threshold
        self.current_index = 0
        self.clap_listeners = set()
        self.clap_sequence_listeners = set()
        self.double_clap_listeners = set()

        # Double clap state
        self.double_clap_min_gap = double_clap_min_gap
        self.double_clap_max_gap = double_clap_max_gap
        self.last_clap_time = None

    @staticmethod
    def note_lengths_to_normalized_pauses(note_lengths):
        note_lengths = note_lengths[:-1]  # slice instead of mutating the original
    ...
    def on_double_clap(self, fn):
        self.double_clap_listeners.add(fn)

    def _check_double_clap(self, time):
        if self.last_clap_time is not None:
            gap = time - self.last_clap_time
            if self.double_clap_min_gap <= gap <= self.double_clap_max_gap:
                for fn in self.double_clap_listeners:
                    fn(time)  # ? pass time into the callback
                self.last_clap_time = None
                return
        self.last_clap_time = time
    ...
    def clap(self, time):
        ...
        if self.min_pattern_time <= time_diff <= self.max_pattern_time:
            ...
            for i in range(self.current_index - self.buffer_size + 1, self.current_index):
                clap_time_diff = self.clap_times[i + 1] - self.clap_times[i] # type: ignore
